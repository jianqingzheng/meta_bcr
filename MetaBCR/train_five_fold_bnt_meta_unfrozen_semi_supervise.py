...
import config_five_fold_bnt_meta_unfrozen_semi_supervise as _cfg_
...
def train(num_fold=None):
    ...
    for fold in all_folds:
        ...
        fdir_train_sars_1 = _cfg_.root_dir + f"data/20240521_rbd_flu_hiv_train_data/20240521-abag_sars-neu_trainmeta_pos_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"
        fdir_train_sars_0 = _cfg_.root_dir + f"data/20240521_rbd_flu_hiv_train_data/20240521-abag_sars-neu_trainmeta_neg_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"

        fdir_val_sars_0 = _cfg_.root_dir + f"data/20240521_rbd_flu_hiv_train_data/20240521-abag_sars-neu_val_neg_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"
        fdir_val_sars_1 = _cfg_.root_dir + f"data/20240521_rbd_flu_hiv_train_data/20240521-abag_sars-neu_val_pos_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"

        fdir_train_nolabel = _cfg_.root_dir + 'data/1222_merged_A1-A11_BCR.xlsx'
        ...
        fdir_tst_neu_BNT = _cfg_.root_dir + "data/240521_testset_BNT162b2_vaccine_neu.csv"
        ...
        data_val_sars_1 = read_table(fdir_val_sars_1)
        data_val_sars_0 = read_table(fdir_val_sars_0)

        data_val = pd.concat([data_val_sars_1, data_val_sars_0], ignore_index=True)
        ...
        data_test_neu_BNT = read_table(fdir_tst_neu_BNT)
        test_name_BNT = 'BNT162b2'
        test_set_neu_BNT = Ab_Dataset(datalist=[data_test_neu_BNT], proportions=[None], sample_func=['sample'],
                                  n_samples=data_test_neu_BNT.shape[0], is_rand_sample=False,
                                  onehot=_cfg_.use_onehot, rand_shift=False)
        test_loader_neu_BNT = DataLoader_n(dataset=test_set_neu_BNT, batch_size=_cfg_.batch_sz, num_workers=0, shuffle=False)
        ...
        for epoch in range(_cfg_.num_epochs):
            ...
            if _cfg_.benchmark == 'acc':
                if (epoch >= _cfg_.saveaft or epoch == 0) and (eval_confusion_val[0] > best_val_acc):
                    ...
                    model.eval()
                    predictions_tst, labels_tst, lossweight_tst = implement(model, test_loader_neu_BNT, )
                    model.train()
                    data_test_neu_BNT['output'] = np.around(np.array(predictions_tst)).tolist()
                    data_test_neu_BNT['predict'] = predictions_tst
                    data_test_neu_BNT.to_excel(
                        f"{_cfg_.date_step}_BNT_neu_results_semi/{test_name_BNT}_{model_name}_BNT_neu_test.xlsx",
            ...
            # add a regular save here for regular check
            if epoch % 5 == 0:
                torch.save(model.state_dict(), f'{_RESULT_DIR}regular-epoch_{epoch}.pth')
                model_name = f'{_cfg_.date}_{epoch}_fold{fold}-maml'
                model.eval()
                predictions_tst, labels_tst, lossweight_tst = implement(model, test_loader_neu_BNT, )
                model.train()
                ...
                data_test_neu_BNT['predict'] = predictions_tst
                data_test_neu_BNT.to_excel(
                    f"{_cfg_.date_step}_BNT_neu_results_semi/regular-{test_name_BNT}_{model_name}_BNT_neu_test.xlsx",
                    index=False, header=True)
