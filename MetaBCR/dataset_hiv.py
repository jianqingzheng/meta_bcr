...
class Ab_Dataset_mean_teacher(Dataset_n):
    ...
    def __getitem__(self, index):
        ...
        if self.onehot:
            ...
            ids_ab_v[0:0 + len(seq_ab_v), :] = one_hot_encoder(s=seq_ab_v)
            ids_ab_l[0:0 + len(seq_ab_l), :] = one_hot_encoder(s=seq_ab_l)
            input_ids_ab_v = torch.tensor(ids_ab_v, dtype = torch.float32)
            input_ids_ab_l = torch.tensor(ids_ab_l, dtype=torch.float32)

        # random mask off noise
        random_noise_flag = random.random()
        if random_noise_flag < 0.5:
            input_ids_ab_v_aug = self.random_maskoff_noise(input_ids_ab_v, len(list(seq_ab_v)))
            input_ids_ab_l_aug = self.random_maskoff_noise(input_ids_ab_l, len(list(seq_ab_l)))
        else:
            input_ids_ab_v_aug = input_ids_ab_v.clone().detach()
            input_ids_ab_l_aug = input_ids_ab_l.clone().detach()
        ...
        if self.onehot:
            ...
            return {'input_ids_ab_v': torch.tensor(input_ids_ab_v_aug),
                    'attention_mask_ab_v': torch.tensor(attention_mask_ab_v),
                    'input_ids_ab_l': torch.tensor(input_ids_ab_l_aug),
                    'attention_mask_ab_l': torch.tensor(attention_mask_ab_l),
