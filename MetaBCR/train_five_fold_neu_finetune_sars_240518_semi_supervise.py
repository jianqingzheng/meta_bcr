...
2024.06.05
'''
import config_five_fold_neu_finetune_sars_240518_semi_supervise as _cfg_
...
def fast_learn(model, sup_criterion, fast_opt, teacher_model=None, unsup_criterion=None, X=None, Y=None, ab_bert=None, mask=None):
    model.train()
    fast_opt.zero_grad()
    outputs = model(ag_x=X[4], ab_x=X[0], attention_mask_ab_v=X[1],
                    ab_l=X[2], attention_mask_ab_l=X[3], ab_bert=ab_bert)
    ...
    if mask is None:
        ...
        teacher_outputs = teacher_model(ag_x=X[4], ab_x=X[5], attention_mask_ab_v=X[1],
                                        ab_l=X[6], attention_mask_ab_l=X[3], ab_bert=ab_bert)
        teacher_model.train()

        unsupervised_loss = inverse_mask * unsup_criterion(outputs[0], teacher_outputs[0])

        loss = torch.mean(supervised_loss + unsupervised_loss)
...
def train():
    for fold in [4]:
        ...
        fdir_tst_hiv = _cfg_.root_dir + f'data/20240510_rbd_flu_hiv_test_data/hiv_unique_test_randomseed-{_cfg_.hiv_split_seed}.xlsx'
        fdir_train_hiv_1 = _cfg_.root_dir + f"data/20240510_rbd_flu_hiv_train_data/20240510-abag_hiv_train_pos_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"
        fdir_train_hiv_0 = _cfg_.root_dir + f"data/20240510_rbd_flu_hiv_train_data/20240510-abag_hiv_train_neg_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"

        fdir_val_hiv_1 = _cfg_.root_dir + f"data/20240510_rbd_flu_hiv_train_data/20240510-abag_hiv_val_pos_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"
        fdir_val_hiv_0 = _cfg_.root_dir + f"data/20240510_rbd_flu_hiv_train_data/20240510-abag_hiv_val_neg_fold{fold}_unique_randomseed-{_cfg_.hiv_split_seed}.xlsx"
        ...
        _RESULT_DIR = _cfg_.root_dir + 'rslt-meta_{}_{}_{}_{}_fold{}_meta{}-meanteacher-trial/'.format(_cfg_.model,
                                                                                                       _cfg_.date,
                                                                                                       _cfg_.train_mode,
                                                                                                       _cfg_.prop, fold,
        ...
        data_train_hiv_1 = read_table(fdir_train_hiv_1)
        data_train_hiv_0 = read_table(fdir_train_hiv_0)
        ...
        data_train_nolabel = read_table(fdir_train_hiv_1)  # TODO: no label data

        train_set = Ab_Dataset_mean_teacher(datalist=[data_train_hiv_1,
                                                      data_train_hiv_0,
                                                      [data_train_hiv_1, data_train_non_experiment],
                                                      [data_train_hiv_1, data_train_non_experiment_flu],
                                                      data_train_nolabel
                                                      ],
                                            ...
                                            n_samples=data_train_hiv_1.shape[0] + data_train_hiv_0.shape[0],
        ...
        data_val_hiv_1 = read_table(fdir_val_hiv_1)
        data_val_hiv_0 = read_table(fdir_val_hiv_0)
        ...
        data_test_hiv = read_table(fdir_tst_hiv)
        test_name = 'testset'
        test_set_hiv = Ab_Dataset(datalist=[data_test_hiv], proportions=[None], sample_func=['sample'],
                                  n_samples=data_test_hiv.shape[0], is_rand_sample=False,
                                  onehot=_cfg_.use_onehot, rand_shift=False)
        test_loader_hiv = DataLoader_n(dataset=test_set_hiv, batch_size=_cfg_.batch_sz, num_workers=0, shuffle=False)
        ...
        if 'unfrozenbert' in _cfg_.date:
            ab_bert = get_unfrozen_bert("prot_bert", n=20)
        else:
            ab_bert = get_frozen_bert("prot_bert")
        ab_bert.to(_cfg_.device)
        ...
        if _cfg_.pretrain_model_dir is not None:
            ...
            model.load_state_dict(torch.load(_cfg_.pretrain_model_dir), strict=False)  # 0107
        else:
        ...
        for epoch in range(_cfg_.num_epochs):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                # fast learning
                fast_loss = fast_learn(fast_model, supervise_criterion, fast_optimizer,
                                       model, unsupervise_criterion,
                                       X=[input_ids_ab_v, attention_mask_ab_v,
                                          input_ids_ab_l, attention_mask_ab_l,
                                          input_ids_ag,
                                          input_ids_ab_v_origin, input_ids_ab_l_origin],
                                       Y=labels, ab_bert=ab_bert, mask=has_label_mask)
                # update slow model
                if i % _cfg_.meta_update_iter == 0:
                    ...
                    outputs = model(ag_x=input_ids_ag, ab_x=input_ids_ab_v, attention_mask_ab_v=attention_mask_ab_v,
                                    ab_l=input_ids_ab_l, attention_mask_ab_l=attention_mask_ab_l, ab_bert=ab_bert)
                ...
                if global_iter % _cfg_.regul_step == (_cfg_.regul_step - 1):
                    ...
                    # validation
                    model.eval()
                    predictions_val, labels_val, lossweight_val = implement(model, meta_loader, ab_bert=ab_bert)
                    model.train()
            ...
            # validation
            model.eval()
            predictions_val, labels_val, lossweight_val = implement(model, val_loader, ab_bert=ab_bert)
            model.train()
            # get validate confusion matrix
            confusion_mat_val = get_confusion_mat(
                [predictions_val[i] for (i, v) in enumerate(lossweight_val) if v == 1],
            ...
            if _cfg_.benchmark == 'acc':
                if (epoch >= _cfg_.saveaft or epoch == 0) and (eval_confusion_val[0] > best_val_acc):
                    ...
                    model.eval()
                    predictions_tst, labels_tst, lossweight_tst = implement(model, test_loader_hiv, ab_bert=ab_bert)
                    model.train()
                    data_test_hiv['output'] = np.around(np.array(predictions_tst)).tolist()
                    data_test_hiv['predict'] = predictions_tst
                    data_test_hiv.to_excel(
                        f"0320_hiv_neu_results/{test_name}_{model_name}_hiv_neutralization_test.xlsx",
