...
def train(num_fold=None, _cfg_=None, antigen_name='rsv'):
    if num_fold == None:
        all_folds = [0, 1, 2, 3, 4]
    ...
    for fold in all_folds:
        ...
        data_train_1 = utils.read_tables(fdir_train_1)
        data_train_0 = utils.read_tables(fdir_train_0)

        data_train_non_experiment = utils.read_tables(fdir_train_non_experiment)
        data_train_non_experiment_sars = utils.read_tables(fdir_train_non_experiment_sars)
        data_train_non_experiment_flu = utils.read_tables(fdir_train_non_experiment_flu)
        data_train_non_experiment_hiv = utils.read_tables(fdir_train_non_experiment_hiv)
        data_train_non_experiment_cross = utils.read_tables(fdir_train_non_experiment_cross)

        data_train_nolabel = utils.read_tables(fdir_train_nolabel)
        ...
        data_val_1 = utils.read_tables(fdir_val_1)
        data_val_0 = utils.read_tables(fdir_val_0)
        ...
        data_test = utils.read_tables(fdir_tst)
        test_name = 'testset'
        ...
        for epoch in range(_cfg_.num_epochs):
            ...
            if _cfg_.benchmark == 'acc':
                if (epoch >= _cfg_.saveaft or epoch == 0) and (eval_confusion_val[0] > best_val_acc):
                    ...
                    data_test.to_excel(
                        f"{_RESULT_DIR}{test_name}_{model_name}_{antigen_name}_binding_test.xlsx",
...
if __name__ == '__main__':
    ...
    config_date = '250217'
    # config_date = '250218'
    configure = get_config(f"Config/config_five_fold_{antigen_name}_bind_meta_{config_date}_semi_supervise.json")

    train(num_fold=None, _cfg_=configure,antigen_name=antigen_name)
